...
static state_t last_state = STATE_GESCHLOSSEN;
...
{
	command_t cmd;
	if (k_pipe_read(&command_pipe, (uint8_t *)&cmd, sizeof(command_t), K_NO_WAIT) ==
	    sizeof(command_t)) {
		switch (cmd) {
		case CMD_OEFFNE_PAKET:
			if (current_state == STATE_GESCHLOSSEN) {
				...
				// STATE_PAKET_GESPERRT: Das Paket muss erst vom Besitzer
