...
#endif
static const struct gpio_dt_spec hall_zu = GPIO_DT_SPEC_GET_OR(HALL_ZU_NODE, gpios, {0});
...
#endif
static const struct gpio_dt_spec hall_p_auf = GPIO_DT_SPEC_GET_OR(HALL_P_AUF_NODE, gpios, {0});
...
#endif
static const struct gpio_dt_spec hall_b_auf = GPIO_DT_SPEC_GET_OR(HALL_B_AUF_NODE, gpios, {0});
...
#endif
static const struct gpio_dt_spec paket_auf = GPIO_DT_SPEC_GET_OR(PAKET_AUF_NODE, gpios, {0});

#define BRIEF_AUF_NODE	DT_ALIAS(briefauf)
#if !DT_NODE_HAS_STATUS_OKAY(BRIEF_AUF_NODE)
#error "Unsupported board: hallbauf devicetree alias is not defined"
#endif
static const struct gpio_dt_spec brief_auf = GPIO_DT_SPEC_GET_OR(BRIEF_AUF_NODE, gpios, {0});
...
void hall_change(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
...
int inputs_init(void)
{
	...
	if (ret != 0) {
		printk("Error %d: failed to configure %s pin %d\n",
		       ret, hall_zu.port->name, hall_zu.pin);
		return -1;
	}

    ret = gpio_pin_interrupt_configure_dt(&hall_zu,
	...
	if (ret != 0) {
		printk("Error %d: failed to configure %s pin %d\n",
		       ret, hall_p_auf.port->name, hall_p_auf.pin);
		return -1;
	}

    ret = gpio_pin_interrupt_configure_dt(&hall_p_auf,
					      GPIO_INT_EDGE_TO_ACTIVE);
	if (ret != 0) {
		printk("Error %d: failed to configure interrupt on %s pin %d\n",
			ret, hall_p_auf.port->name, hall_p_auf.pin);
	...
	if (ret != 0) {
		printk("Error %d: failed to configure %s pin %d\n",
		       ret, hall_b_auf.port->name, hall_b_auf.pin);
		return -1;
	}

    ret = gpio_pin_interrupt_configure_dt(&hall_b_auf,
					      GPIO_INT_EDGE_TO_ACTIVE);
	if (ret != 0) {
		printk("Error %d: failed to configure interrupt on %s pin %d\n",
			ret, hall_b_auf.port->name, hall_b_auf.pin);
	...
	if (ret != 0) {
		printk("Error %d: failed to configure %s pin %d\n",
		       ret, paket_auf.port->name, paket_auf.pin);
		return -1;
	}

    ret = gpio_pin_interrupt_configure_dt(&paket_auf,
					      GPIO_INT_EDGE_TO_ACTIVE);
	if (ret != 0) {
		printk("Error %d: failed to configure interrupt on %s pin %d\n",
			ret, paket_auf.port->name, paket_auf.pin);
	...
	if (ret != 0) {
		printk("Error %d: failed to configure %s pin %d\n",
		       ret, brief_auf.port->name, brief_auf.pin);
		return -1;
	}

    ret = gpio_pin_interrupt_configure_dt(&brief_auf,
					      GPIO_INT_EDGE_TO_ACTIVE);
	if (ret != 0) {
		printk("Error %d: failed to configure interrupt on %s pin %d\n",
			ret, brief_auf.port->name, brief_auf.pin);
