...
void motor_main(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		...
		if (k_pipe_read(&motor_set_pipe, (uint8_t *)&my_motor_set, sizeof(motor_set_t),
				K_NO_WAIT) == sizeof(motor_set_t)) {
...
int motor_init(void)
{
	...
	k_poll_event_init(&motor_adc_event, K_POLL_TYPE_SIGNAL, K_POLL_MODE_NOTIFY_ONLY,
			  &motor_adc_done_signal);
